...
namespace DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        void Inserts<T>(IDbConnection connection, IEnumerable<T> entities, IDbTransaction transaction, int? commandTimeout) where T : class;
        ...
        bool Delete<T>(IDbConnection connection, Expression<Func<T, bool>> exp, IDbTransaction transaction, int? commandTimeout) where T : class;

        bool Delete<T>(IDbConnection connection, IPredicate predicate, IDbTransaction transaction, int? commandTimeout) where T : class;

        bool Deletes<T>(IDbConnection connection, string ids, string cName, IDbTransaction transaction, int? commandTimeout) where T : class;
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
            var properties = classMap.ListProperty.Where(p => p.KeyType != KeyType.NotKey);
        ...
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
            List<IPropertyMap> nonIdentityKeyProperties = classMap.ListProperty.Where(p => p.KeyType == KeyType.Guid || p.KeyType == KeyType.Assigned).ToList();
            var identityColumn = classMap.ListProperty.SingleOrDefault(p => p.KeyType == KeyType.Identity);
        ...
        public bool Deletes<T>(IDbConnection connection, string ids, string cName, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                else
                {
                    ...
                    return connection.Execute(string.Format("delete from {0} where {1} in ({2})", typeof(T).Name, string.Join(",", strArr), cName), null, transaction, commandTimeout, CommandType.Text) > 0;
                }
            }
        ...
        public bool Delete<T>(IDbConnection connection, Expression<Func<T, bool>> exp, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            string str = SqlExpession.Where<T>(exp);

            if (!string.IsNullOrEmpty(str))
            {
                return connection.Execute(string.Format("delete from {0} where {1} ", typeof(T).Name, str), null, transaction, commandTimeout, CommandType.Text) > 0;
            }
            return false;
        }

        public bool Delete<T>(IDbConnection connection, IClassMapper classMap, IPredicate predicate, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();
        ...
        {
            var whereFields = classMap.ListProperty.Where(p => p.KeyType != KeyType.NotKey);
            if (!whereFields.Any())
            {
        ...
        public int Update<T>(IDbConnection connection, object updateDict, object keyDict, IDbTransaction trans, int? timeout) where T : class
        {
            ...
            foreach (var item in d2)
            {
                ...
                if (tp.Equals(typeof(string)) || tp.Equals(typeof(DateTime)))
                {
                    s2.Append(item.Name + "='" + item.GetValue(keyDict, null) + "' and ");
                }
                else
                {
                    s2.Append(item.Name + "=" + item.GetValue(keyDict, null) + " and ");
