...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        string GetOrderBy(IClassMapper classMap);
    }
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string Select(IClassMapper classMap, IPredicate predicate, IList<ISort> sort, IDictionary<string, object> parameters)
        {
            ...
            string bsc = BuildSelectColumns(classMap);
            string btn = GetTableName(classMap);
            StringBuilder sql = new StringBuilder(string.Format("SELECT {0} FROM {1}", bsc, btn));
            if (predicate != null)
            {
                sql.Append(" WHERE ")
